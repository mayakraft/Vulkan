...
Engine::Engine() {
  ...
  VkMemoryAllocateInfo vertAllocInfo{};
  vertAllocInfo.sType = VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO;
  vertAllocInfo.allocationSize = memRequirements.size;
  vertAllocInfo.memoryTypeIndex = findMemoryType(memRequirements.memoryTypeBits, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT);

  if (vkAllocateMemory(logicalDevice, &vertAllocInfo, nullptr, &vertexBufferMemory) != VK_SUCCESS) {
